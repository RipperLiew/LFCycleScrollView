...
// MARK:- pageControl 相关
private let LFPageControlMargin: CGFloat = 15
private let LFPageControlPointWidth: CGFloat = 2
...
protocol PageControlAlimentProtocol {
    //协议中的声明的属性必须要显式的 {get} or {get set} specifier
    var pageControlAliment: PageControlAliment { get set }
    var pageControlPointSpace: CGFloat { get set }
    func relayoutPageControl(pageControl:LFPageControl)
    func relayoutPageControl(pageControl:LFPageControl,outerFrame:CGRect)
}

extension PageControlAlimentProtocol where Self : UIView {
    //只给遵守PageControlAlimentProtocol协议的UIView添加了拓展
    ...
    func relayoutPageControl(pageControl: LFPageControl) {
        if pageControl.isHidden == false {
            ...
            switch self.pageControlAliment {
                ...
                case.RightBottom:
                    pageX = bounds.width - pageW - LFPageControlMargin
                case.LeftBottom:
                    pageX = bounds.origin.x + LFPageControlMargin
            }
